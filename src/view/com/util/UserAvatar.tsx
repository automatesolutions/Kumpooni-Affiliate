...
let UserAvatar = ({
  ...
  const t = useTheme()
  const backgroundColor = t.atoms.bg_contrast_25.backgroundColor
...
let EditableUserAvatar = ({
  ...
  const aviStyle = useMemo(() => {
    if (type === 'algo' || type === 'list' || type === 'store') {
      return {
        width: size,
        height: size,
        borderRadius: size > 32 ? 8 : 3,
  ...
  const onOpenLibrary = React.useCallback(async () => {
    ...
    // processPhoto(image)
    // onSelectNewAvatar(image)
    try {
      const croppedImage = await openCropper({
        mediaType: 'photo',
        height: type === 'banner' ? 240 : image.height,
        width: type === 'banner' ? 690 : image.width,
        path: image.path,
      })
      ...
      <Menu.Trigger label={`Edit avatar`}>
        {({ props }) => (
          <TouchableOpacity
            ...
            <View style={[styles.editButtonContainer, t.atoms.bg_contrast_25]}>

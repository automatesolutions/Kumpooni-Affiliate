...
import { android, atoms as a, flatten, tokens, useTheme } from '#/theme'
...
export function Button({
  ...
  const { baseStyles, hoverStyles } = React.useMemo(() => {
    ...
    } else if (color === 'primary_blue') {
      if (variant === 'solid') {
        if (!disabled) {
          baseStyles.push({
            backgroundColor: t.palette.blue_500,
          })
          hoverStyles.push({
            backgroundColor: t.palette.blue_600,
          })
        } else {
          baseStyles.push({
            backgroundColor: t.palette.blue_700,
          })
        }
      } else if (variant === 'outline') {
        baseStyles.push(a.border, t.atoms.bg, {
        ...
        if (!disabled) {
          baseStyles.push(a.border, {
            borderColor: t.palette.blue_500,
          })
          hoverStyles.push(a.border, {
            backgroundColor: light ? t.palette.blue_50 : t.palette.blue_950,
          })
        } else {
          baseStyles.push(a.border, {
            borderColor: light ? t.palette.blue_200 : t.palette.blue_900,
          })
        }
      } else if (variant === 'ghost') {
        if (!disabled) {
          baseStyles.push(t.atoms.bg)
          hoverStyles.push({
            backgroundColor: light ? t.palette.blue_100 : t.palette.blue_900,
          })
        }
      }
    }
  ...
  const { gradientColors, gradientHoverColors, gradientLocations } =
    React.useMemo(() => {
      ...
      const gradient = {
        ...
        primary_blue: tokens.gradients.sky,
      }[color || 'primary']

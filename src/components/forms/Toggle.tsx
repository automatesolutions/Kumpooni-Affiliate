...
export function createSharedToggleStyles({
  ...
  if (selected) {
    base.push({
      backgroundColor:
        t.name === 'light' ? t.palette.blue_25 : t.palette.blue_900,
    ...
    if (hovered) {
      baseHover.push({
        backgroundColor:
          t.name === 'light' ? t.palette.blue_100 : t.palette.blue_800,
        borderColor:
          t.name === 'light' ? t.palette.blue_600 : t.palette.blue_400,
      })
    }
  } else {
    if (hovered) {
...
export function Checkbox() {
  ...
  return (
    <View
      ...
      {selected ? <Checkmark size="xs" fill={t.palette.blue_500} /> : null}
    </View>
  )
}

...
export function ServicesSelectDialog({
  control,
  onClose,
  onSelect,
}: ServicesSelectProps) {
  ...
  const onSelectService = useCallback(
    (service: Services) => {
      control.close(() => onSelect(service))
    },
    [control, onSelect],
  )
  return (
    <Dialog.Outer
      ...
      <ErrorBoundary renderError={renderErrorBoundary}>
        <ServicesList onSelect={onSelectService} control={control} />
      </ErrorBoundary>
    </Dialog.Outer>
  )
}
...
function ServicesList({
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['filtered-services'],
    queryFn: async () => {
      if (!session?.store_id) {
        throw new Error('Failed to fetch store services')
      }
      const { data, error } = await supabase.rpc('get_filtered_services', {
        store_id_param: session.store_id,
      })

      if (error) {
        logger.error('Services Offer', error)
        throw error
      }
  ...
  const renderItem = useCallback(
    ({ item }: { item: Services }) => {
      return <ServiceItem key={item.id} service={item} onSelect={onSelect} />
    },
    [onSelect],
  )
...
function ServiceItem({
  ...
  return (
    <TouchableOpacity
      ...
      <Text style={[a.text_xl]}>{service.name}</Text>
    </TouchableOpacity>
  )
}
